...
export function ImageDropZone({
  ...
  const zoneCls = [
    ...
    disabled ? "opacity-60" : "focus-within:ring-1 focus-within:ring-carrot-orange/20",
    dragOver && !disabled
      ? "ring-2 ring-carrot-orange/50 bg-olive/5"
      : !compact
        ? "border border-dashed border-olive/15 hover:border-olive/30"
        : "",
    className,
  ]
    .filter(Boolean)
  ...
  return (
    ...
    >
      ...
      {!compact && !disabled && (
        <p className="text-[9px] text-ivory-mist/40 mt-2 pointer-events-none select-none">
          Arrastra imágenes aquí · clic para enfocar y pegar (⌘V)
        </p>
      )}

      {dragOver && !disabled && (
        <div className="absolute inset-0 z-10 flex items-center justify-center rounded-md bg-shadow-grey/75 border-2 border-carrot-orange/40 pointer-events-none">
          <span className="text-[11px] tracking-[0.2em] uppercase text-ivory-mist">
      ...
      {uploading && (
        <div className="absolute inset-0 z-20 flex items-center justify-center rounded-md bg-shadow-grey/80 pointer-events-none">
          <span className="text-[11px] tracking-[0.2em] uppercase text-ivory-mist/90">
            Subiendo…
          </span>
        </div>
      )}
